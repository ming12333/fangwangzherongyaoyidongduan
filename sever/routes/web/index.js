module.exports=app=>{
    ...
    router.get('/news/init',async(req,res)=>{
       const parent=await Category.findOne({
        ...
        const newsTitles=[" 从人们的日常生产和生活中就可以看到","，，最终将会让通信企业以最低的成本获得最高的效益。  2.25G通信技术的优势  5G通信技术具",
                       "最终将会让通信企业以最低的成本获得最高的效益。  2.25G通信技术的优势  5G通信技术具","最终将会让通信企业以最低的成本获得最高的效益。  2.25G通信技术的优势  5G通信技术具"]
        const newsList = newsTitles.map(title => {
            const randomCats = cats.slice(0).sort((a, b) => Math.random() - 0.5)
            //cats.slice(0)cats中复制出一份数据sort 排序((a,b)=>Math.random()-0.5)
           //sort若 a 小于 b，在排序后的数组中 a 应该出现在 b 之前，则返回一个小于 0 的值。
//若 a 等于 b，则返回 0。
...
return {
        ...
        await Article.deleteMany({})//对数据库进行清空
    ...
    router.get('/news/list',async(req,res)=>{ 
         ...
         // path:'children',
         //   populate:{
          //       path:"newsList"//子分类的newslist
         //   }
        // }).lean()//展示关联的子分类
        // res.send(parent)
       const cats=await Category.aggregate([//聚合查询
       { $match:{parent:parent._id}},//过滤数据 $match条件查询，parent=parent._id
     {
            $lookup:{
    ...
    {
          $addFields:{//本意为添加字段，在这里是修改字段
            newsList:{$slice:['$newsList',5]}//只要5条$slice操作符，表示取几个

        }
        }
    ])
    const subCats = cats.map(v => v._id)//查找id
    cats.unshift({
      name: '热门',
      newsList: await Article.find().where({
        categories: { $in: subCats }//$in特殊操作符,subCats子分类
      }).populate('categories').limit(5).lean()
    })//populate('categories')关联的分类

    cats.map(cat1 => {//循环查找
      cat1.newsList.map(news => {
        news.categoryName1 = (cat1.name === '热门')
          ? news.categories[0].name : cat1.name
        return news
      })
      return cat1
    })
...
router.get('/articles/:id', async (req, res) => {
  ...
  }).limit(2)// data.related相关的文章
  //categories: { $in: data.categories }相同分类的
  res.send(data)
})
